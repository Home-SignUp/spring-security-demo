...
public class SecurityConfig extends WebSecurityConfigurerAdapter { // можно использовтаь механизм адаптера
     ...
     *
     * НЕдостаток такого способа в том, что здесь представление генериться на только сервере И если это использовать совместно с AngularJS - то такая архитектура уже НЕподходит (потому-что представление генерится уже на нескольких сторонах: и на стороне сервера и на стороне AngularJS...)
     */
    @Override
